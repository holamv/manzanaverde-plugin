...
- **v1.13.0 (2026-08-12): canal ambiguo → preguntar, no asumir + Leopoldo Anastacio en LEADERS.** Cuando el routing cae en el default de prioridad 4 (`#issues-líderes`/`#issues-general`, sin `es_weekly`/area software/`cr_pair`), el skill debe preguntar al usuario a qué canal enviar el CR según la posición del owner, en vez de publicar directo — motivado por un CR real que llegó a `#issues-líderes` sin contexto claro. Agregado Leopoldo Anastacio (Chef Ejecutivo) al mapping `LEADERS`.
